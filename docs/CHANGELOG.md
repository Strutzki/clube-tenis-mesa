...
## 2026-09-06
- **Experiência do admin — Despachos como tela inicial + contadores nas abas** — (1) o card "📋 Despachos do dia" agora abre e carrega sozinho ao entrar no painel (antes exigia um toque em "Abrir"), então o admin já cai no que precisa de ação, por circuito. (2) A barra de abas do admin ganhou selo com o nº de pendências do circuito selecionado: "Inscr." mostra inscrições aguardando aprovação; "Pend." mostra partidas aguardando validação + solicitações de W.O. em aberto ("9+" acima de 9). Ambos só-leitura, footprint-zero. (Só front — `atualizar.sh`.)

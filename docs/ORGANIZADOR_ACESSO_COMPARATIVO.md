...
- **Fatia 2 — Financeiro por circuito. ✅ FEITA (migração live; edge/front na fonte).** Coluna `circuitos.org_ve_financeiro` (default OFF, **com grant ao anon na mesma migração** — lição do incidente) + ação super-admin `DEFINIR_ORG_VE_FINANCEIRO` + gate condicional no enforcement (ações financeiras do organizador só passam com o flag ON) + `LOGIN_ORGANIZADOR` devolve `veFinanceiro` + front: card do super-admin (`FinanceiroOrgCard`) e BottomNav esconde a aba "Pagam." do organizador quando OFF. Inerte (sem organizador em produção); edge deploya com o 1º circuito vendido.

...
// A senha do admin NAO mora mais aqui. Ate 08/09/2026 ela era uma constante neste
// arquivo — e este arquivo e entregue a qualquer visitante do site, entao a senha
// do super-admin estava publica, e como ela e a mesma que vai ao servidor como PIN,
// qualquer pessoa podia agir como admin por chamada direta, sem passar pelo app.
// Agora o app nao confere senha nenhuma: manda ao servidor e o SERVIDOR decide.
...
// Confere a senha do admin CONTRA O SERVIDOR, com uma acao de leitura pura
// (LISTAR_ORGANIZADORES nao grava nada). Fail-closed: se nao der para conferir,
// nao entra. O servidor tem trava de 5 tentativas em 15 minutos — por isso o
// erro de senha e devolvido com a mensagem que o proprio servidor mandou.
async function conferirSenhaAdmin(senha) {
  const corta = new AbortController();
  const relogio = setTimeout(() => corta.abort(), 12000); // rede ruim nao pode travar a tela
  try {
    const res = await fetch(`${SUPA_URL}/functions/v1/admin-action`, {
      method: "POST",
      signal: corta.signal,
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${SUPA_KEY}`, "apikey": SUPA_KEY },
      body: JSON.stringify({ pin: senha, acao: "LISTAR_ORGANIZADORES", payload: {} }),
    });
    const dados = await res.json().catch(() => ({}));
    if (dados && dados.sucesso) return { ok: true };
    return { ok: false, motivo: (dados && dados.erro) || "Senha incorreta." };
  } catch (e) {
    return { ok: false, motivo: "Não deu para conferir a senha agora. Verifique a internet e tente de novo." };
  } finally {
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "RENOVAR": {
      // Intenção de renovar para a próxima temporada (não mexe em pagamento).
      const { athleteId, quer } = action.payload;
    ...
    case "LIBERAR_NAO_RENOVANTES": {
      // Gated no servidor (só depois do prazo). Aqui não mexemos no estado —
      // o loadFromSupabase que roda em seguida traz o resultado real.
      return state;
    }
...
function LoginScreen({ onLogin, onAthleteLogin, onVisitante, athletes, onInscricao, onOrganizadorLogin }) {
  ...
  if (mode === "admin") return (
    <AdminLoginBiometria
      ...
      doAdmin={doAdmin} conferindo={conferindo}
  ...
  async function doAdmin(bypass=false) {
    ...
    if (conferindo) return;                       // Enter repetido nao dispara duas vezes
    if (user !== ADMIN_USER) { setErr("Usuário ou senha incorretos."); setTimeout(()=>setErr(""),3000); return; }
    setConferindo(true);
    setErr("");
    const r = await conferirSenhaAdmin(pass);
    setConferindo(false);
    if (r.ok) { setPinCache(pass); onLogin(); }
    else { setErr(r.motivo); setTimeout(()=>setErr(""),5000); }
  }
}
...
function AdminLoginBiometria({ s, LOGO, user, setUser, pass, setPass, err, setErr, doAdmin, conferindo, onBack, onOrganizador }) {
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.card}>
        ...
        <input style={{...s.input,marginBottom:10,letterSpacing:2,fontSize:18,textAlign:"center",fontWeight:800}}
          type="password" autoComplete="current-password"
          value={pass} onChange={e=>setPass(e.target.value)}
          placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&doAdmin()}/>
        <button style={s.btn()} onClick={() => doAdmin()} disabled={conferindo}>{conferindo ? "Conferindo..." : "Entrar"}</button>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(17,28,25,0.92)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:340,background:T.verdeCard,borderRadius:18,padding:24,border:`1px solid ${T.bordaSuave}`,textAlign:"center",boxSizing:"border-box"}}>
        ...
        <input
          ...
          type="password" autoComplete="current-password" placeholder="••••••••" autoFocus
          onKeyDown={e=>e.key==="Enter" && confirmar()}
          style={{width:"100%",boxSizing:"border-box",background:T.verde,border:`1px solid ${T.borda}`,borderRadius:10,color:T.offwhite,padding:"14px",fontFamily:T.mono,fontSize:18,textAlign:"center",letterSpacing:2,outline:"none",marginBottom:16}}/>
...
export default function App() {
  ...
  async function syncToSupabase(action, st) {
    ...
    // Sai no mesmo clique que abre o WhatsApp — precisa do mesmo tratamento.
      ...
      const { athleteId, quer } = action.payload;
      await chamarAtletaAction("RENOVAR", { athleteId, quer });
      await loadFromSupabase();
    }
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
        {[
          {label:"Atletas no circuito",val:`${ativos.length}/${state.maxAtletas||20}`,color: ativos.length>=(state.maxAtletas||20)?"#c25a45":"#D85A30"},
...
function RenovacaoAdminPanel({ state, dispatch }) {
  ...
  return (
    <Card style={{marginTop:8,border:"1px solid rgba(167,139,250,0.28)"}}>
      ...
      {!dataIni ? (
        <div style={{fontSize:11,color:"#9C6F3E"}}>Defina a data de início em "Gerar próxima temporada" para abrir a janela de renovação.</div>
      ) : (
        <>
          ...
          <div style={{display:"flex",gap:14,flexWrap:"wrap",fontSize:12,marginBottom:10}}>
            ...
            <span style={{color:"#9C6F3E"}}>⏳ Sem resposta: <b>{naoRenov.length}</b></span>
            <span style={{color:"#7d9188"}}>💵 Pagos: <b>{pagos.length}</b></span>
          </div>
          {naoRenov.length > 0 && (
            <div style={{fontSize:11,color:"#9db3a8",marginBottom:10}}>
